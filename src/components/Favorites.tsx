      ...
      <div className="relative bg-dark-card border border-dark-border rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-dark-border">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-dark-text">收藏夹</h2>
          ...
          {notes.length === 0 ? (
            ...
            <div className="divide-y divide-dark-border">
              {notes.map(note => (
                <div 
                  key={note.id}
                  className="p-4 hover:bg-dark-bg/50 transition-colors cursor-pointer group"
                  onClick={() => {
                    onNoteClick(note)
                    onClose()
                  }}
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-[10px] text-dark-muted">{formatTime(note.updatedAt)}</span>
                      </div>
                    </div>
                    <button
                      onClick={(e) => {
                      ...
                      className="p-1.5 rounded hover:bg-dark-border text-yellow-400 opacity-0 group-hover:opacity-100 transition-opacity"
                      title="取消收藏"
                    >
